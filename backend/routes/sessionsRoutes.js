...
// Get statistics (FIXED - Simple version)
router.get('/stats', (req, res) => {
  db.all('SELECT status, COUNT(*) as count FROM training_sessions GROUP BY status', (err, rows) => {
    if (err) {
    ...
    if (rows && rows.length > 0) {
      rows.forEach(row => {
        ...
        if (row.status === 'done') stats.completed = row.count;
        if (row.status === 'archived') stats.archived = row.count;
